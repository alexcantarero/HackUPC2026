...
void GeneticAlgorithm::run(std::atomic<bool>& stop_flag) {
    ...
    for (int i = 0; i < hp.population_size; ++i) {
        if (stop_flag.load()) break; // Hard exit check
    ...
    while (!stop_flag.load()) {
        ...
        if (best_.score < previous_global_best.score) {
            stagnation = 0;
            // Removed the debug print here so it doesn't spam, 
            // updateBest in the parent class handles the live terminal output!
        } else {
            ++stagnation;
        }
        ...
        while (static_cast<int>(next_population.size()) < target_children) {
            ...
            Chromosome child = crossoverOX1(population[parent_a].chromosome, population[parent_b].chromosome);
    ...
    std::cout << "[DEBUG] " << name() << " safely returning at Gen " << generation << ".\n";
}
...
GeneticAlgorithm::Chromosome GeneticAlgorithm::crossoverOX1(const Chromosome& p1, const Chromosome& p2) {
...
// ---> THIS WAS THE MISSING FUNCTION THAT BROKE THE COMPILER! <---
...
bool GeneticAlgorithm::isBetterSolution(const Solution& lhs, const Solution& rhs) const {
    ...
    if (lhs.score + kScoreEps < rhs.score) return true;
    if (rhs.score + kScoreEps < lhs.score) return false;
    
    // Tie breaker 1: More bays is better
    if (lhs.bays.size() != rhs.bays.size()) {
        return lhs.bays.size() > rhs.bays.size();
    }
    
    // Tie breaker 2: Smaller overall footprint is better (packs tighter)
